...
let totalQuesCount =  `<span>`+index+`<span> of </span><span>`+questions.length+`</span> Fragen </span>`;
...
if(score >6){
let scoreTag = `<span>Ausgezeichnet! `+score+` von `+questions.length+` 🔥</span>`;  
scoreText.innerHTML = scoreTag;
}else if(score >5){
let scoreTag = `<span> Gut ` + score + ` von ` + questions.length + ` 💪</span>`;
scoreText.innerHTML = scoreTag;       
}else{
let scoreTag = `<span>Nächstes mal wird besser! `+score+` von `+questions.length+` 😉</span>`;  
scoreText.innerHTML = scoreTag;       
}
};